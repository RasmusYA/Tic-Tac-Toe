						...
						if (!starter) {
							while (!addSlot) {
								try {
									...
									// Om platsen är tom retunerar den sant.
									String userInput = SelectSlot.selectSlot(scanner);
									addSlot = GameBoard.AddToGameBoard(ConvertInput.convertInputToInt(userInput),
											playerTag, true);
								}
								// OM användaren matar in något annat än mellan 1-3
								catch (ArrayIndexOutOfBoundsException ex) {
							...
							while (!enemyAddSlot) {
								...
								EnemyLines.EnemySass();
								// enemyAddSlot = GameBoard.AddToGameBoard(Enemy.enemyTurn(), enemyTag, false);
								enemyAddSlot = GameBoard.AddToGameBoard(EnemySmarter.enemyAI(playerTag, enemyTag),
										enemyTag, false);
							}
						}
						GameBoard.showGameBoard();
